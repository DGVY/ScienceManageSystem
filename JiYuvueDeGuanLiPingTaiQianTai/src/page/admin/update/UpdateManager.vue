<template>
    <div>
        <div>
            <el-button size="small" type="primary" @click="handleQuery"v-auth code="account:projectUpdate:list">刷新列表</el-button>
            <el-button size="small" type="primary" @click="showApplicationDetails"v-auth code="account:projectUpdate:detail">查看申请</el-button>
            <el-button size="small" type="primary" @click="showProject" v-auth code="account:project:detail">查看项目</el-button>
            <el-button size="small" type="primary" @click="applyPass"v-auth code="account:projectUpdate:pass" >申请通过</el-button>
            <el-button size="small" type="danger" @click="applyNotPass" v-auth code="account:projectUpdate:notpass">申请驳回</el-button>
        </div>
        <br/>
        <el-table
            :data="tableData"
        ...
        <el-dialog title="变更驳回" :visible.sync="updateApplicationNotPassVisible" size="small" >
            <el-form :model="form" >
                ...
                <el-form-item label="驳回理由" :error="errors.name">
                    <el-input type="textarea"  :autosize="{ minRows: 5}" v-model="form.backInformation" placeholder="大致描述你要变更的内容" ></el-input>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="handleNotPass">申请</el-button>
        ...
        <el-dialog title="变更详情" :visible.sync="detailDialogVisible" size="small" >
            <el-form :model="form" >
                ...
                <el-form-item label="变更说明" :error="errors.name">
                    <el-input type="textarea"  v-model="form.updateDescription"  readonly="readonly" ></el-input>
                </el-form-item>
                <el-form-item label="驳回理由" :error="errors.name">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            handleQuery() {
                this.$axios.get(this.$config.SYSTEM_HOST + "/update_application/query").then((res) => {
                    this.tableData = res.data.data.rows;
                    this.totalCount = res.data.data.totalCount;
                });
            },
            applyPass(){
                if(this.currentRow.stats == '申报中') {
                    this.$axios.put(this.$config.SYSTEM_HOST + "/update_application/pass", this.currentRow).then((res) => {
                        this.handleQuery();
                    });
                }else {
            ...
            handleNotPass(){
                this.$axios.put(this.$config.SYSTEM_HOST + "/update_application/not_pass", this.form).then((res) => {
            ...
            showApplicationDetails(){
                this.$objectUtils.clear(this.errors);
                this.$objectUtils.clear(this.form);
                this.$axios.get(this.$config.SYSTEM_HOST + "/update_application?id=" + this.currentRow.id).then((res) => {
